...
const getCinemaRooms = async (req, res, next) => {
  try {
    ...
    const list = await CinemaRoom.findAll({ where: filter, order: [['name', 'ASC']] });
...
const addCinemaRoom = async (req, res, next) => {
  try {
    ...
    const cinema = await Cinema.findOne({ where: { id: cinema_id, is_deleted: false } });
    if (!cinema) {
...
const updateCinemaRoom = async (req, res, next) => {
  try {
    ...
    await room.update({ ...req.body, updated_by: req.user?.id || null });
    return res.status(200).json({ success: true, data: room });
  } catch (error) {
    next(error);
  }
...
const deleteCinemaRoom = async (req, res, next) => {
  try {
    ...
    await room.update({ is_deleted: true, updated_by: req.user?.id || null });
